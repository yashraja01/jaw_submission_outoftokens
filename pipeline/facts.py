"""Resolve the entities a question mentions to the entities the corpus actually holds.

Questions are written the way a bid desk writes them - "irr & waterways dept rajasthan", "gujarat
pw", "mega infra authority", "ut pr pkg 2 wtp augmentation". Matching is therefore fuzzy on the
question side and exact on the corpus side: every canonical entity is scored and the best one is
taken, and free text is never accepted as an entity in its own right.

This module is deliberately model-free. The language model decides *what* a question is asking;
which client that is remains a scoring problem over the corpus, where a wrong pick is invisible -
correct arithmetic over the wrong client passes every range check there is.
"""
import re
import sqlite3
...
def _dec(v):
    """SQLite keeps every amount as text; a missing one reads back as 'None'."""
    try:
        return Decimal(v)
...
# The geography of the country, not of any question set: a client's state is part of how it is
# named ("Public Works Department, Govt of West Bengal") and questions abbreviate it. Any state
# actually present in the corpus is added to this list at load time, so an estate covering states
# not listed here still resolves.
STATES = ["uttar pradesh", "madhya pradesh", "west bengal", "tamil nadu", "maharashtra",
          "gujarat", "rajasthan", "jharkhand", "odisha", "delhi", "andhra pradesh", "assam",
          "bihar", "chhattisgarh", "goa", "haryana", "himachal pradesh", "karnataka", "kerala",
          "manipur", "meghalaya", "mizoram", "nagaland", "punjab", "sikkim", "telangana",
          "tripura", "uttarakhand", "arunachal pradesh", "jammu and kashmir", "ladakh",
...
class Facts:
    def __init__(self, db_path):
        ...
        for w in self.works:
            ...
            w["work_name"] = w["work_name"] or ""
            w["category"] = w["category"] or ""
            w["year"] = int(w["completed"][:4]) if re.match(r"\d{4}", w["completed"] or "") else 0
        self.by_pkg = {w["pkg"]: w for w in self.works}

        # The state list is the country's, extended by whatever this estate actually names, so a
        # corpus covering states the base list omits still resolves them.
        seen_states = {(w["state"] or "").strip().lower() for w in self.works}
        self.states = sorted({s for s in STATES} | {s for s in seen_states if s},
        ...
        for k, c in self.clients.items():
            ...
            state = next((s for s in self.states if s in norm_q(c["name"])), None)
            statewords = {w for s in self.states for w in s.split()}
        ...
        self.categories = sorted({w["category"] for w in self.works if w["category"]},
                                 key=len, reverse=True)
        self.bonds = [dict(r) for r in self.db.execute("SELECT * FROM bond")]
        self.dossiers = [dict(r) for r in self.db.execute("SELECT * FROM dossier")]
        for row in self.bonds:
            row["amount"] = _dec(row["amount"])
        for row in self.dossiers:
            row["bid_value"] = _dec(row["bid_value"])
    ...
    def resolve_work(self, q, manager=None):
        ...
        state = next((s for s in self.states if s in nq), None)
        best, score = None, 0.0
        for w in pool:
            wn = norm_q(w["work_name"])
            head = [t for t in wn.split() if t not in STOP and not t.isdigit() and t != "pkg"
                    and t not in {x for s in self.states for x in s.split()}]
